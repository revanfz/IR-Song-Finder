...
def cari_lagu():
    ...
    if query == "":
        ...
        if len(lagu) == 0:
            ...
            hasil.tkraise()
            Label(hasil, bg="teal", fg="white", text="Song Title : ",  font=("Times",20,"bold")).pack(pady=20)
            Label(hasil, font=("Times", 18), text=lagu[0][0]).pack()
            Button(hasil, font=("Times",14,"bold"), text="Search again", command=lambda:hasil.destroy()).pack(pady=10)
            Button(hasil, font=("Times",14,"bold"), text="Close", command=lambda:window.destroy()).pack()
